...
class Point {
    ...
    constructor(pos: PositionOptions) {
        this.x = pos.x;
        this.y = pos.y;
    }

    nextMove(dir: CardinalDirection) {
...
class PointDynamics {
    point: Point;
    lastDeflection: DeflectionOption;
    direction: Direction;
    cardinal: CardinalDirection;
    constructor(p: Point, dir: CardinalDirection) {
        this.point = p;
    ...
    setLastDeflection(option: DeflectionOption) {
...
class Block {
    ...
    constructor(position: PositionOptions, id: number) {
...
class BlockSystem {
    blocks: Block[]
    startPositon: PositionOptions;
    lines: number;
    blockLength: number;
    blockHeight: number


    constructor(start: PositionOptions, lines: number, length: number, height: number) {
        this.blocks = []
    ...
    addBlock(position: PositionOptions) {
 ...
 * ENUM SECTION
 */
enum DeflectionOption {
    Side,
    Block,
    Bat, None
}

enum BatMovementOption {
    Left,
    Right
}
...
interface DeflectionType {
    eventOption: DeflectionOption
}
...
class Board {
    ...
    checkIfBlockHitBall(position: PositionOptions, id: number, active: boolean) {
        ...
        var offset = (this.gameOptions.ballSize / 2) - 3
        if (last === DeflectionOption.Block) return
